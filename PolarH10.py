...
class PolarH10:
    ...
    ELECTRO_SERVICE_UUID = "0000feee-0000-1000-800-00805f9b34fb"
    ...
    def __init__(self, bleak_device):
        ...
        self.ecg_wall_times = []  # host-time seconds, aligned per ECG sample
        self.acc_data = None
        self.ibi_data = None
    ...
    # 00 = ECG; EA 1C AC CC 99 43 52 08 = last sample timestamp in nanoseconds; 00 = ECG frameType, sample0 = [68 00 00] microVolts(104) , sample1, sample2, ....
        ...
        if data[0] == 0x00:
            ...
            while offset < len(samples):
                ...
                wall_t = recv_t - (n_samples - 1 - i) * time_step
                self.ecg_wall_times.extend([wall_t])
                sample_timestamp += time_step
                i += 1
    ...
    async def start_ecg_stream(self):
        ...
        print("Collecting ECG data (PMD)...", flush=True)
    ...
    def get_ecg_data(self):
        ...
        wall = np.array(self.ecg_wall_times) if self.ecg_wall_times else np.array([])
        self.ecg_data = {
            "times": ecg_times,
            "wall_times": wall,
            "values": np.array(self.ecg_stream_values),
        }
